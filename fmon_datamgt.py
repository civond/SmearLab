...
#Record summary for session at beginning
...
def write_file(file,localtime):
    sess = open(file,'a')
    sess.write(localtime.strftime('%Y-%m-%dT%H_%M_%S')) #video location
    sess.close()
...
# Record data compilation at the end of a session 
def data_comp(data):
    data_handle = file(compdatafile,'a')
    np.savetxt(data_handle,[data],delimiter=',',fmt="%s")
    data_handle.close()
...
# Update values for each trial by reading in randomized order
...
# Increment Reporting Statistics
